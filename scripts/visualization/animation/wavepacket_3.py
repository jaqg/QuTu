...
def Eoa(n,alfa):
    res = (n + 1/2) * alfa/mu # Resultado en unidades atomicas
    return res
# Variable de desplazamiento xi
def xi(x,alfa):
    return np.sqrt(alfa) * x
# Constante de normalizacion
def N(n,alfa):
    res = ( np.sqrt(alfa/np.pi) * (1/(2**n * factorial(n))) )**(1/2)
    return res
# Funcion exponencial
def expo(x,alfa):
    return np.exp(-(xi(x,alfa)**2)/2)
...
def time_expo(n,t,alfa):
    return np.exp(-1j * Eoa(n,alfa) * t/hbar_J)
# Funciones propias
def phi(n,x,t,alfa):
    return N(n,alfa) * expo(x,alfa) * Hv(n,x) * time_expo(n,t,alfa)
# Densidad de probabilidad
def dens_phi(n,x,t,alfa):
    return np.conj(phi(n,x,t,alfa)) * phi(n,x,t,alfa)
...
def Phi1(x,t,alfa):
    ...
    for i in np.arange(0,len(cPhi1)):
        if abs(cPhi1[i])>limite_coefs:
            # print("i =", i, "factorial(i) =", factorial(i))
            res = cPhi1[i] * phi(2*i+1,x,t,alfa) + res
    res = res
    return res
def dPhi1(x,t,alfa):
    res = np.conj(Phi1(x,t,alfa)) * (Phi1(x,t,alfa))
    return res
#
def Phi2(x,t,alfa):
    res = 0
    for i in np.arange(0,len(cPhi2)):
        if abs(cPhi2[i])>limite_coefs:
            # print("i =", i, "factorial(i) =", factorial(i))
            res = cPhi2[i] * phi(2*i,x,t,alfa) + res
    res = res
    return res
def dPhi2(x,t,alfa):
    res = np.conj(Phi2(x,t,alfa)) * (Phi2(x,t,alfa))
    return res
#
def Phi3(x,t,alfa):
    res = 0
    for i in np.arange(0,len(cPhi3)):
        if abs(cPhi3[i])>limite_coefs:
            # print("i =", i, "factorial(i) =", factorial(i))
            res = cPhi3[i] * phi(2*i+1,x,t,alfa) + res
    res = res
    return res
def dPhi3(x,t,alfa):
    res = np.conj(Phi3(x,t,alfa)) * (Phi3(x,t,alfa))
    return res
#
def Phi4(x,t,alfa):
    res = 0
    for i in np.arange(0,len(cPhi4)):
        if abs(cPhi4[i])>limite_coefs:
            # print("i =", i, "factorial(i) =", factorial(i))
            res = cPhi4[i] * phi(2*i,x,t,alfa) + res
    res = res
    return res
def dPhi4(x,t,alfa):
    res = np.conj(Phi4(x,t,alfa)) * Phi4(x,t,alfa)
    return res
...
def Psi(x,t,alfa,alfa_grad):
    res = c0(alfa_grad) * Phi0(x,t,alfa) + \
          c1(alfa_grad) * Phi1(x,t,alfa) + \
          c2(alfa_grad) * Phi2(x,t,alfa) + \
          c3(alfa_grad) * Phi3(x,t,alfa)
    return res
def dPsi(x,t,alfa,alfa_grad):
    res = np.conj(Psi(x,t,alfa,alfa_grad)) * Psi(x,t,alfa,alfa_grad)
    return res
...
for i in range(13):
    alfa_ind = i
    alfa_grad = alfa_grad_arr[alfa_ind]
    energia = E_alfa[alfa_ind]/EUACM
    x1 = pcx1[alfa_ind]
    x2 = pcx2[alfa_ind]
    x3 = pcx3[alfa_ind]
    x4 = pcx4[alfa_ind]
    ...
    def actualizar(i):
        t = t0 + (dt * i)
        #
        y1 = dPsi(x_vec,t,alfa_a0,alfa_grad).real*multiplicador + energia
        y2 = Psi(x_vec,t,alfa_a0,alfa_grad).real*multiplicador + energia
        ...
        #
        ax.plot(x_A/RBOHR,V/EUACM,label=r'$V\,(x)$',color='gray',lw=1)
        #
        # Niveles de energia
        for i in np.arange(0,len(E)):
            if E[i]<=Vb:
                ax.hlines(y = E[i]/EUACM, xmin=-x1c(i), xmax=-x2c(i), color='gray', lw=1.0)
                ax.hlines(y = E[i]/EUACM, xmin=x2c(i), xmax=x1c(i), color='gray', lw=1.0)
            else:
                ax.hlines(y = E[i]/EUACM, xmin=-x1c(i), xmax=x1c(i), color='gray', lw=1.0)
        ax.hlines(y = energia, xmin=x1, xmax=x2, color='black', lw=1.3)
        ax.hlines(y = energia, xmin=x3, xmax=x4, color='black', lw=1.3)
        #
        plt.title(r'$\alpha = {}$,'.format(alfa_grad) + '\t' + r'$t = {:.4f}$ ps'.format(t))
