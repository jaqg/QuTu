...
program qutu
    ...
    call parse_args(input_filename)

    ! Read INPUT file (default or user-supplied)
    call read_input_file(trim(input_filename), input_params, ierr)
...
contains
    ...
    subroutine parse_args(input_filename)
        character(len=256), intent(out) :: input_filename
        character(len=256) :: arg
        integer :: nargs

        input_filename = 'INPUT'   ! default
    ...
    subroutine print_sample_input()
        write(*,'(A)') '# ============================================================='
        write(*,'(A)') '# QuTu INPUT file — generated by: qutu --help'
        write(*,'(A)') '# Format: key = value   (comments start with #)'
        write(*,'(A)') '# Usage:'
        write(*,'(A)') '#   qutu                 run with INPUT in current directory'
        write(*,'(A)') '#   qutu myfile          run with myfile as the input'
        write(*,'(A)') '#   qutu -h              print this template to stdout'
        write(*,'(A)') '#   qutu -h > INPUT      generate a starter INPUT file'
        write(*,'(A)') '# ============================================================='
        write(*,'(A)') ''
        write(*,'(A)') '# --- Basis set ---'
        write(*,'(A)') '# basis = HO   : harmonic oscillator (default)'
        write(*,'(A)') '# basis = PIB  : particle-in-a-box finite basis representation'
        write(*,'(A)') 'basis = HO'
        write(*,'(A)') ''
        write(*,'(A)') '# Number of basis functions (higher = more accurate but slower)'
        write(*,'(A)') 'N_max = 100'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Spatial grid (atomic units: a0) ---'
        write(*,'(A)') 'xmin = -5.0'
        write(*,'(A)') 'xmax =  5.0'
        write(*,'(A)') 'dx   =  0.02'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Potential (choose one block) ---'
        write(*,'(A)') ''
        write(*,'(A)') '# Option A: polynomial  V(x) = sum_k v_k * x^k'
        write(*,'(A)') '# poly_degree = 4'
        write(*,'(A)') '# v_coeffs    = 0.0, 0.0, -0.5, 0.0, 0.1   # v0..v4'
        write(*,'(A)') '# mass        = 1.0                          # reduced mass (amu)'
        write(*,'(A)') ''
        write(*,'(A)') '# Option B: NH3-style double well  V(x) = Vb*(x^2/xe^2 - 1)^2'
        write(*,'(A)') '# xe     = 0.3816   # equilibrium position (Angstrom)'
        write(*,'(A)') '# Vb     = 2028.6   # barrier height (cm^-1)'
        write(*,'(A)') '# mass_H = 1.00782503207'
        write(*,'(A)') '# mass_N = 14.0030740048'
        write(*,'(A)') ''
        write(*,'(A)') '# --- PIB-specific (only needed when basis = PIB) ---'
        write(*,'(A)') '# box_length = 10.0   # box size L (a0); default: xmax - xmin'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Optional ---'
        write(*,'(A)') '# print_level = 1    # 0=minimal  1=full OUTPUT (default)  2=+dat files'
        write(*,'(A)') '# alpha       = 4.7  # HO width parameter (auto-computed if omitted)'
        write(*,'(A)') '# alpha_values = 0.0, 45.0, 90.0   # mixing angles for wavepacket (deg)'
    end subroutine print_sample_input
