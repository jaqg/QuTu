...
program doble_pozo_NH3
    ...
    real*8, dimension(:), allocatable :: dpsi1_vec,dpsi2_vec,dpsi3_vec,dpsi4_vec
    real*8, dimension(:), allocatable :: psi1_vec_A,psi2_vec_A,psi3_vec_A,psi4_vec_A
    real*8, dimension(:), allocatable :: dpsi1_vec_A,dpsi2_vec_A,dpsi3_vec_A,dpsi4_vec_A
    ...
    do i=90, 102, 1
        write(i,9999)
        write(i,'("Psi = cos(alfa)/dsqrt(2)*(Phi0 + Phi1) + sen(alfa)/dsqrt(2)*(Phi2 + Phi3)")')
        write(i,9998)
        write(i,'("alfa",6x,"E (cm-1)",6x,"x (A)",9x,"psi(x)",8x,"|psi(x)|^2")')
    end do
    ...
    do_N: do N=2, Nmax, 1
        ...
        if (N==Nmax) then
            do i=1, steps_x+1, 1
                ...
                do j=1, Npar, 1
                    ...
                    if (abs(H_par(j,1))>0.001) Phi0=Phi0+(-H_par(j,1))*phi(2*(j-1),alpha,x)
                    if (abs(H_par(j,2))>0.001) Phi2=Phi2+(H_par(j,2))*phi(2*(j-1),alpha,x)
                    if (abs(H_par(j,3))>0.001) Phi4=Phi4+(H_par(j,3))*phi(2*(j-1),alpha,x)
                    !
                    if (abs(H_par(j,1))>0.001) Phi0_A=Phi0_A+(-H_par(j,1))*phi(2*(j-1),alpha/RBOHR**2,x_angstrom)
                    if (abs(H_par(j,2))>0.001) Phi2_A=Phi2_A+(H_par(j,2))*phi(2*(j-1),alpha/RBOHR**2,x_angstrom)
                    if (abs(H_par(j,3))>0.001) Phi4_A=Phi4_A+(H_par(j,3))*phi(2*(j-1),alpha/RBOHR**2,x_angstrom)
                end do
                Phi0_vec(i) = Phi0
                Phi2_vec(i) = Phi2
                Phi4_vec(i) = Phi4
        ...
        !
            ...
            ! do i = 1, 10
            !     write(*,'(1000000f6.2)') ( c_k(i,j), j=1, 4 )
            ! end do
            ...
            allocate(min_coef(N), max_coef(N))
            min_coef = minval(H_par, 1) ! Valor minimo de cada columna
            max_coef = maxval(H_par, 1) ! Valor maximo de cada columna
            ! Si |min|>|max|, entonces el signo de los coeficientes esta
            ! alterado, y usamos -H. En caso contrario, usamos H.
            ! Elementos de indice par
            ! do j = 1, Npar
            !     if (abs(min_coef(j))>abs(max_coef(j))) then
            !         do i = 1, Npar
            !             c_k(2*i-1,2*j-1) = -H_par(i,j)
            !         end do
            !     else
            ...
            ! min_coef = minval(H_impar, 1) ! Valor minimo de cada columna
            ! max_coef = maxval(H_impar, 1) ! Valor maximo de cada columna
            ! do j = 1, Nimpar
            !     if (abs(min_coef(j))>abs(max_coef(j))) then
            !         do i = 1, Nimpar
            !             c_k(2*i,2*j) = -H_impar(i,j)
            ...
            !     else
            !         do i = 1, Nimpar
            !             c_k(2*i,2*j) = H_par(i,j)
            !         end do
            !     end if
            ! end do
            ! do i = 1, 10
            !     write(*,'(1000000f6.2)') ( c_k(i,j), j=1, 10 )
            ! end do
        end if
        ...
        if (N==Nmax) then
            do i=1, steps_x+1, 1
                ...
                do j=1, Nimpar, 1
                    ...
                    ! los coeficientes de la combinacion c<0.001 no
                    ! contribuyan a la combinacion
                    if (abs(H_impar(j,1))>0.001) Phi1=Phi1+(-H_impar(j,1))*phi(2*j-1,alpha,x)
                    if (abs(H_impar(j,2))>0.001) Phi3=Phi3+(-H_impar(j,2))*phi(2*j-1,alpha,x)
                    if (abs(H_impar(j,3))>0.001) Phi5=Phi5+(H_impar(j,3))*phi(2*j-1,alpha,x)
                    if (abs(H_impar(j,1))>0.001) Phi1_A=Phi1_A+(-H_impar(j,1))*phi(2*j-1,alpha/RBOHR**2,x_angstrom)
                    if (abs(H_impar(j,2))>0.001) Phi3_A=Phi3_A+(-H_impar(j,2))*phi(2*j-1,alpha/RBOHR**2,x_angstrom)
                    if (abs(H_impar(j,3))>0.001) Phi5_A=Phi5_A+(H_impar(j,3))*phi(2*j-1,alpha/RBOHR**2,x_angstrom)
                end do
                Phi1_vec(i) = Phi1
                Phi3_vec(i) = Phi3
                Phi5_vec(i) = Phi5
        ...
        if (N==Nmax) then
            ...
            do i=1, steps_x+1, 1
                ...
                psi1_vec_A(i) = c_psi1(1)*Phi0_vec_A(i) + c_psi1(2)*Phi1_vec_A(i)
                psi2_vec_A(i) = c_psi1(1)*Phi0_vec_A(i) - c_psi1(2)*Phi1_vec_A(i)
                psi3_vec_A(i) = c_psi2(1)*Phi2_vec_A(i) + c_psi2(2)*Phi3_vec_A(i)
                psi4_vec_A(i) = c_psi2(1)*Phi2_vec_A(i) - c_psi2(2)*Phi3_vec_A(i)
        ...
        if (N==Nmax) then
            ...
            ! do i = 1, 10
            !     write(*,'(10000f15.5)') ( mat_x_arm(i,j), j=1, 10 )
            ! end do
            ...
            ! Valores de alfa = 0,10,15,20,30,40,45,50,60,6570,80 y 90
            ...
            do j=1, size(array_alfa_4EE)
                ...
                !
                t0 = 0.d0
                ! tf disminuye con alfa, de t=100 (alfa=0) a t=1 (alfa=90)
                ! tf = 100.d0 - 8.d0*(j-1)
                ! if (j<=7) tf = 100.d0 - 8.d0*(j-1)
                ! if (j>7) tf = 1.d0
                tf = 50.d0
                ! steps_t = 500
                ! dt = (tf - t0)/(steps_t*1.d0)
                dt = 5.d-3
                steps_t = int((tf - t0)/dt)
                allocate(t_vec(steps_t))
                write(109+j,9999)
